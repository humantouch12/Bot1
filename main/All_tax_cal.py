...
init()
def Tax_value():
  print("------------------------------------------------------------ Total  Value of Tax  is about to be checked ----------------------------------------------")
  try: 
  #creating pairs 
    ...
    with open("data/totaltax.json", 'w') as json_file:
        json.dump(total_asset, json_file)


    print(Fore.YELLOW +  f" Overall Tax is : {total_asset} USDT")
    print(total_values)
  except Exception as e:
    errorcode = 1
    with open("counters/error.json", 'w') as json_file:
      json.dump(errorcode, json_file)
    print( Fore.RED + f"an exception error has occured in Tax_value {e}  ")
