...
result_queue = Queue()
def Second_trade():
  print("-----------------------------------------Second Trade Has started--------------------------------------------------------------------")
  try:
  ...
  # changes  from made here
    ...
    if trade_pair.startswith(Current_crypto):
      ...
      trade_quantity = Main_account * (1/(float(details[1]['price'])))
      side ="BUY"
    ...
    try:
      ...
      if 'status' in trading_data  and trading_data ['status'] == 200:
        print(Fore.GREEN +" second  trade was successful ")
      ...
      print(Fore.RED +f"an exception error has occured in second trade{e}")
